...
add_audit("companies", len(companies), 0, "Success")
...
sectors = pd.read_excel(DATA_DIR / "sectors.xlsx")
...
add_audit("sectors", len(sectors), 0, "Success")


analysis = pd.read_excel(DATA_DIR / "analysis.xlsx", header=1)

analysis = analysis[analysis["company_id"].isin(companies["id"])]

analysis.to_sql("analysis", conn, if_exists="replace", index=False)

print("Analysis inserted successfully!")

add_audit("analysis", len(analysis), 0, "Success")


balancesheet = pd.read_excel(DATA_DIR / "balancesheet.xlsx", header=1)

# Keep only valid company IDs
balancesheet = balancesheet[balancesheet["company_id"].isin(companies["id"])]

# Remove duplicate company-year records
balancesheet = balancesheet.drop_duplicates(subset=["company_id", "year"], keep="first")
...
cashflow = cashflow[cashflow["company_id"].isin(companies["id"])]
...
profitandloss = profitandloss[profitandloss["company_id"].isin(companies["id"])]
...
financial_ratios = pd.read_excel(DATA_DIR / "financial_ratios.xlsx")
...
financial_ratios.to_sql("financial_ratios", conn, if_exists="replace", index=False)

print("Financial Ratios inserted successfully!")

add_audit("financial_ratios", len(financial_ratios), 0, "Success")

market_cap = pd.read_excel(DATA_DIR / "market_cap.xlsx")

market_cap.to_sql("market_cap", conn, if_exists="replace", index=False)

print("Market Cap inserted successfully!")

add_audit("market_cap", len(market_cap), 0, "Success")

peer_groups = pd.read_excel(DATA_DIR / "peer_groups.xlsx")

peer_groups.to_sql("peer_groups", conn, if_exists="replace", index=False)

print("Peer Groups inserted successfully!")

add_audit("peer_groups", len(peer_groups), 0, "Success")

stock_prices = pd.read_excel(DATA_DIR / "stock_prices.xlsx")

stock_prices.to_sql("stock_prices", conn, if_exists="replace", index=False)
...
pd.DataFrame(audit).to_csv(OUTPUT_DIR / "load_audit.csv", index=False)
