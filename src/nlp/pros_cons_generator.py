...
df = df.sort_values("year").groupby("company_id").tail(1).reset_index(drop=True)
...
pros_cons = pros_cons[pros_cons["confidence_pct"] > 60]
...
existing = pros_cons.groupby(["company_id", "type"]).size().unstack(fill_value=0)
...
pros_cons.to_csv(OUTPUT_DIR / "pros_cons_generated.csv", index=False)
...
summary.to_csv(OUTPUT_DIR / "pros_cons_validation.csv")
...
print("Companies Missing Pros :", (~summary["Has_Pro"]).sum())

print("Companies Missing Cons :", (~summary["Has_Con"]).sum())
