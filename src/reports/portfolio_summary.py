...
def load_company(company_id):
    ...
    sector = sector_table[sector_table["company_id"] == company_id].reset_index(
        drop=True
    )
...
def generate_portfolio_summary():
    ...
    for _, row in companies.iterrows():
        ...
        if len(sector) > 0:
            ...
            pdf.drawRightString(
                RIGHT, PAGE_HEIGHT - 55, f"Sector : {sector.iloc[0]['broad_sector']}"
            )
        ...
        pdf.drawString(LEFT, 720, "Portfolio KPI Summary")

        pdf.line(LEFT, 710, RIGHT, 710)
        ...
        if len(intelligence) > 0:
            ...
            quality = str(intelligence.iloc[0]["capital_allocation_label"])
        ...
        kpis = [
            ...
            (
                ...
                trend_arrow(latest["revenue_cagr_5yr"], previous["revenue_cagr_5yr"]),
            ),
            (
                "PAT CAGR",
                f"{latest['pat_cagr_5yr']:.2f}%",
                trend_arrow(latest["pat_cagr_5yr"], previous["pat_cagr_5yr"]),
            ),
            ("Free Cash Flow", fcf, "-"),
        ...
        pdf.drawString(50, y, "KPI")

        pdf.drawString(240, y, "Value")

        pdf.drawString(420, y, "Trend")
        ...
        for name, value, arrow in kpis:
            ...
            pdf.drawString(240, y, str(value))
        ...
        pdf.drawString(LEFT, 20, f"Page {page_no} of {len(companies)}")

        pdf.drawRightString(RIGHT, 20, "N100 Financial Intelligence")
