...
cashflow_intelligence = pd.read_excel(OUTPUT_DIR / "cashflow_intelligence.xlsx")

pros_cons = pd.read_csv(OUTPUT_DIR / "pros_cons_generated.csv")
...
def load_company(company_id):
    ...
    ratios = (
        financial_ratios[financial_ratios["company_id"] == company_id]
        .sort_values("year")
    ...
    capital = cashflow_intelligence[
        cashflow_intelligence["company_id"] == company_id
    ].reset_index(drop=True)

    company_pc = pros_cons[pros_cons["company_id"] == company_id]

    pros = company_pc[company_pc["type"] == "Pro"].reset_index(drop=True)

    cons = company_pc[company_pc["type"] == "Con"].reset_index(drop=True)
...
def create_revenue_chart(profit, company_id):
    ...
    plt.bar(chart["year"], chart["sales"])
...
def create_profit_chart(profit, company_id):
    ...
    plt.bar(chart["year"], chart["net_profit"])
...
def create_balance_chart(balance, company_id):
    ...
    chart["equity"] = chart["equity_capital"] + chart["reserves"]

    plt.figure(figsize=(8, 3.5))

    plt.bar(chart["year"], chart["equity"], label="Equity")

    plt.bar(
        chart["year"], chart["borrowings"], bottom=chart["equity"], label="Borrowings"
    )

    chart["other_liabilities"] = (
        chart["total_liabilities"] - chart["equity"] - chart["borrowings"]
    )

    plt.bar(
        chart["year"],
        chart["other_liabilities"],
        bottom=chart["equity"] + chart["borrowings"],
...
if __name__ == "__main__":
    ...
    create_revenue_chart(data["profit"], "TCS")

    create_profit_chart(data["profit"], "TCS")

    create_roe_roce_chart(data["ratios"], data["company"], "TCS")

    create_balance_chart(data["balance"], "TCS")

    create_cashflow_chart(data["cash"], "TCS")
...
def generate_tearsheet(company_id):
    ...
    cash = data["cash"]
    if len(profit) < 3 or len(balance) < 3 or len(cash) < 3:
    ...
    sector = capital.iloc[0]["sector"] if not capital.empty else "Unknown"
    ...
    pdf.drawString(LEFT, PAGE_HEIGHT - 35, company["company_name"])

    pdf.setFont("Helvetica", 12)

    pdf.drawString(LEFT, PAGE_HEIGHT - 55, f"Ticker : {company_id}")

    pdf.drawRightString(RIGHT, PAGE_HEIGHT - 55, f"Sector : {sector}")
    ...
    cards = [
        ...
        ("Revenue CAGR", f"{latest['revenue_cagr_5yr']:.2f}%"),
        ("PAT CAGR", f"{latest['pat_cagr_5yr']:.2f}%"),
        ("Debt / Equity", f"{latest['debt_to_equity']:.2f}"),
        ("Quality", f"{latest['composite_quality_score']:.2f}"),
    ]
    ...
    for i, (title, value) in enumerate(cards):
        ...
        pdf.roundRect(x, y, CARD_WIDTH, CARD_HEIGHT, 6)

        pdf.setFont("Helvetica-Bold", 10)

        pdf.drawString(x + 10, y + 40, title)

        pdf.setFont("Helvetica-Bold", 16)

        pdf.drawString(x + 10, y + 18, str(value))

    # ======================================================
    # CHARTS
    # ======================================================

    revenue_chart = create_revenue_chart(profit, company_id)

    profit_chart = create_profit_chart(profit, company_id)

    roe_chart = create_roe_roce_chart(ratios, company, company_id)

    pdf.setFont("Helvetica-Bold", 15)

    pdf.drawString(LEFT, 430, "Revenue Trend")

    pdf.drawImage(str(revenue_chart), LEFT, 250, width=240, height=150)

    pdf.drawString(310, 430, "Net Profit Trend")

    pdf.drawImage(str(profit_chart), 300, 250, width=240, height=150)

    pdf.drawString(LEFT, 220, "ROE vs ROCE")

    pdf.drawImage(str(roe_chart), LEFT, 10, width=500, height=180)

    pdf.setFont("Helvetica", 9)

    pdf.drawString(LEFT, 20, "Page 1")

    pdf.drawRightString(RIGHT, 20, "Generated by N100 Financial Intelligence")
    ...
    pdf.drawString(LEFT, PAGE_HEIGHT - 55, "Financial Summary")
    ...
    balance_chart = create_balance_chart(balance, company_id)

    pdf.setFont("Helvetica-Bold", 15)

    pdf.drawString(LEFT, 735, "Balance Sheet Composition")

    pdf.drawImage(str(balance_chart), LEFT, 520, width=250, height=180)

    # ======================================================
    # CASH FLOW CHART
    # ======================================================

    cash_chart = create_cashflow_chart(cash, company_id)

    pdf.drawString(310, 735, "Cash Flow")

    pdf.drawImage(str(cash_chart), 300, 520, width=250, height=180)
    ...
    for _, row in pros.iterrows():
        ...
        lines = wrap_text(row["text"], width=40)
    ...
    pdf.roundRect(LEFT, 240, 260, 45, 8, fill=True)

    pdf.setFillColor(colors.white)

    pdf.setFont("Helvetica-Bold", 14)

    pdf.drawCentredString(LEFT + 130, 256, badge)

    pdf.setFillColor(colors.black)

    pdf.setFont("Helvetica-Bold", 15)

    pdf.drawString(LEFT, 190, "About Company")
    ...
    wrapped = textwrap.fill(about, width=85)

    text = pdf.beginText()

    text.setTextOrigin(LEFT, 170)
...
if __name__ == "__main__":
    ...
    pd.DataFrame(skipped).to_csv(OUTPUT_DIR / "skipped_tearsheets.csv", index=False)
