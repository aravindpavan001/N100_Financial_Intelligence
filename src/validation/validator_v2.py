...
for name, df in {
    ...
    if not dup.empty:
        add_failure(
            "DQ-02", "CRITICAL", name, f"{len(dup)} duplicate company-year rows"
        )
...
if not bad.empty:
    add_failure(
        "DQ-04", "WARNING", "balancesheet", f"{len(bad)} balance sheet mismatch"
    )

# DQ-05 OPM cross check
calc = (profitandloss["operating_profit"] / profitandloss["sales"]) * 100
bad = profitandloss[
    (profitandloss["sales"] > 0) & ((calc - profitandloss["opm_percentage"]).abs() > 1)
]
if not bad.empty:
...
# DQ-07
bad = profitandloss[
    (profitandloss["tax_percentage"] < 0) | (profitandloss["tax_percentage"] > 100)
]
if not bad.empty:
